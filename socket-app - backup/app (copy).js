...
try {
    ...
    //io.set("Access-Control-Allow-Methods", "PUT, GET, POST, DELETE, OPTIONS");
        ...
        socket.on('message', (message) => {
            ...
            io.emit('message', {text: message.msg });
        });
        ...
        socket.on("getOnlineUser", data => {
         ...
         socket.on("sendMSG", data => {
         	...
         	/************* Harjinder ****************/
            if(data.from){
            	io.sockets.sockets[data.coach_socket_id].emit("sendMSG", data);
          ...
          console.log(data);
           db.each("SELECT * FROM users WHERE socket_id = " + "\'" + socket.id + "\'", function(err, getuser) {
            ...
            /*db.each("SELECT * FROM users WHERE socket_id = " + "\'" + socket.id + "\'", function(err, getuser) {            	
                if (getuser.client_id == getuser.coatch_id) {
                    db.each("SELECT * FROM users WHERE client_id ="+ "\'" + data.sender_id + "\'", function(err, getsender) {
                        console.log("Message Received from coach app: ", data);
                        io.sockets.sockets[getsender.socket_id].emit("sendMSG", data);
                        io.emit("message", data);
                    })
                } else {
                    db.each("SELECT * FROM users WHERE client_id = "+ "\'" + getuser.coatch_id + "\'",
                        function(err, getsenderClient) {
                        	if(!err){
                        		console.log(getsenderClient);
	                            io.sockets.sockets[getsenderClient.socket_id].emit("sendMSG", data);
                      		}else{
                      			console.log(err);
                      		} 
                      	})
